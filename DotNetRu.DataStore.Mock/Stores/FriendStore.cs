namespace XamarinEvolve.DataStore.Mock.Stores
{
    ...
    public class FriendStore : BaseStore<FriendModel>, IFriendStore
    {
        List<FriendModel> _sponsors;
        ...
        public override async Task<FriendModel> GetItemAsync(string id)
        {
            await this.InitializeStore();
            return this._sponsors.FirstOrDefault(s => s.Id == id);
        }

        public override async Task<IEnumerable<FriendModel>> GetItemsAsync(bool forceRefresh = false)
        {
            await this.InitializeStore();

            var json = Newtonsoft.Json.JsonConvert.SerializeObject(this._sponsors);
            return this._sponsors as IEnumerable<FriendModel>;
        }
        ...
        {
            if (this._initialized) return Task.FromResult(true);

            this._initialized = true;

            this._sponsors = new List<FriendModel>();
